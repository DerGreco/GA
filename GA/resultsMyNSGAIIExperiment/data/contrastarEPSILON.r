...
aplicaContrastes <- function(datos1,datos2){
	...
	}else{ #En caso de no haber normalidad
...
# Este segmento del código es el que se encarga de cargar datos y hacer contrastes

# Archivos a procesar terminados en TXT. Esta cadena se puede cambiar por otra mejor (GREP)
fuentesdatos<-list.files(pattern="EPSILON.res")
...
write.table(resultados, file = "Contrastes.EPSILON", append = FALSE, sep = "\t")
#unlink("Resultados")

write.table(estadisticos, file = "Estadisticos.EPSILON", append = FALSE, sep = "\t")
#unlink("Estadisticos")
